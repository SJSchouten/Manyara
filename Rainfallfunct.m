...
function Rainfallfunct(rmeanwind,ss)
...
data.station{n} = data.station{n}(:,2:13);
    ...
    for b = 1:(12*(length(data.station{1})-1))
        ...
        full.time = (1:1:length(full.station{n}));
...
subplot(((length(data.names)+1)/2),2,n),plot(full.plottime',simpfitting.ff_simple{n},':r'),hold on, plot(full.plottime',input_record,':k');
hold on, plot(full.plottime',movmean(input_record,12),'k'), hold on, plot(full.plottime',movmean(simpfitting.ff_simple{n},12),'r');legend('Fit RMSE = '+string(simpfitting.RMSE{n}),'Raw data','Yearly runmean data','Yearly runmean fit')
grid;title(data.names{n});xlim([1922,2018]);

subplot(((length(data.names)+1)/2),2,6), plot((ss(:,1)/1000),ss(:,2),'r:'), hold on, plot((ss(:,1)/1000),movmean(ss(:,2),365),'k');
...
figure,
for n = 1:length(data.names)
    subplot(2,1,1),plot(wind.time,wind.full{n});grid;xlabel('Month nr.');ylabel('Rainfall monthlysum (mm)');title('Rainfall of all stations over time');
    hold on,plot(wind.time,wind.llv,'k-x'); hold on, legend(data.names)
    hold on,
    subplot(2,1,2),plot(wind.time,wind.fullan{n});grid;xlabel('Month nr.');ylabel('Rainfall monthly anomaly (mm)');title('Rainfall anomaly of all stations over time');
    hold on, legend(data.names)
    hold on,
end
...
for n = 1:length(data.names)
    ...
    hold on, plot(int.time,data.shortrains{n},':'); legend(data.names2)
    hold on,
    subplot(3,1,3),plot(int.time,int.runmeanL{n});grid;xlabel('Year');ylabel('Rainfall longrains (mm)');title('Long');
    hold on, plot(int.time,data.longrains{n},':'); legend(data.names2)
    hold on,
end
