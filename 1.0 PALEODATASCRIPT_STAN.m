% Reading Quartenary data 2-18-2019 Stan Schouten.
...
for lakes = 1 
lakes.man = xlsread('Composite_african_lakes.xlsx','Schouten_2020_Manyara');
lakes.man = man(:,1);
lakes.man = man(:,3);
man_llv = man(:,4);
man_cond = man(:,5); 
man_loi = man(:,6);
lakes.nvic = xlsread('Composite_african_lakes.xlsx','Stager_2005_N-Victoria');
nvic_age = nvic(:,3);
nvic_swd = nvic(:,4);
nvic_cond = nvic(:,5);
lakes.vic = xlsread('Composite_african_lakes.xlsx','Berke_2012_Victoria');
vic_age = vic(:,3);
vic_bit = vic(:,6);
vic_c13 = vic(:,10);
lakes.tang = xlsread('Composite_african_lakes.xlsx','Tierney_2010_Tanganyika');
...
tang_bsi = tang(:,13);
lakes.tang2 = xlsread('Composite_african_lakes.xlsx','Stager_2009_Tanganyika');
tang_cond_age = lakes.tang2(:,2);
tang_cond = lakes.tang2(:,3);
tang_cond_errhigh = tang_cond+(lakes.tang2(:,5));
tang_cond_errlow = tang_cond-(lakes.tang2(:,5));
tang_pol_age = lakes.tang2(:,8);
tang_aqpol = lakes.tang2(:,9);
lakes.naiv = xlsread('Composite_african_lakes.xlsx','Verschuren_2000_Naivasha');
naiv_depth = naiv(:,1);
naiv_llv_age = naiv(:,3);
naiv_llv = naiv(:,5);
...
figure,yyaxis left,plot(steinhilber_age,steinhilber_TSI), hold on, yyaxis right, plot(usoskin_age, usoskin_SN); xlim([0 3000]);
...
pers = sort([1,2,5,10,11,9,100,500,200,400,300,800,1000,2000,200,10,20]);
...
[ff_simple,Mean_comp,Decadal,Centenial,Millenial,Semi_Decadal,RMSE,difference_simple,comps]=Hfitsimple(input_time,input_record);

comps(2,:) = comps
comps(1,(2:length(pers)+1)) = pers
...
disp(string(comps(1,:))+' = '+string(comps(2,:)))
...
figure,yyaxis right,plot(man_depth,man_llv,'r');ylabel('lake level (cm)'),hold on,yyaxis left,plot(naiv_llv_age,naiv_llv,'b'),
legend('Manyara','Naivashia');xlabel('Time B2k');grid;ylabel('lake level (m)');xlim([-20 400])
